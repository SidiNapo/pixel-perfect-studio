...
function Lights() {
  return (
    <>
      ...
      <pointLight position={[8, 4, 6]} intensity={5} color="#8b5cf6" distance={35} decay={2} />
      <pointLight position={[6, -5, 4]} intensity={4} color="#7c3aed" distance={30} decay={2} />
      <pointLight position={[2, 2, 8]} intensity={3} color="#a855f7" distance={25} decay={2} />
      
      {/* Blue accent highlight */}
      <pointLight position={[7, 1, 0]} intensity={2.5} color="#6366f1" distance={20} decay={2} />
      
      {/* Key light from top - white for sharp highlights */}
      <directionalLight position={[4, 12, 6]} intensity={1.5} color="#ffffff" />
      
      {/* Rim light from right - purple tint for edge glow */}
      <directionalLight position={[12, 3, 5]} intensity={1.5} color="#c084fc" />
      
      {/* Fill from front-bottom */}
      <directionalLight position={[3, -6, 8]} intensity={0.8} color="#a78bfa" />
      
      {/* Front face fill */}
      <directionalLight position={[-2, 0, 12]} intensity={0.6} color="#f5f3ff" />
    </>
  );
}
